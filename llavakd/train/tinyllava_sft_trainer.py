"""
SFT Student Trainer - 用于SFT阶段训练Student模型
中间checkpoint: 保存 connector + llm 权重
最终保存: 由 training_recipe.save() 保存完整模型
"""
...
class SFTStudentTrainer(Trainer):
    """
    SFT阶段的Student模型训练器
    - 中间checkpoint: 保存 connector + llm 权重
    - 最终模型: 由 training_recipe.save() 保存完整模型
    """

    def _save_checkpoint(self, model, trial, metrics=None):
        """
        中间checkpoint保存 connector + llm 权重，节省存储空间。
        Vision Tower 不保存（训练时冻结，权重不变）。
        """
        ...
        if self.args.local_rank in [-1, 0]:
            ...
            logger.info(f"✓ Checkpoint saved: connector ({connector_size_mb:.2f} MB) + llm ({llm_size_mb:.2f} MB) -> {output_dir}")
